...
**Information Expert**: ответственность должен брать на себя тот, кто обладает максимумом информации для исполнения.
...
Сочетается с Information Expert и Low coupling (знать об объектах должен тот, кто их использует).  
...
Коррулирует с Information Expert
...
*Популярная проблема квадрат-прямоугольник.*
```
От кого что наследовать?  
Логично, что квадрат от прямоугольника, т.к. квадрат - частный случай прямоугольника.  
Прописав все методы для класса и переопределив метод базового класса "высчитать площадь" в наследнике квадрате,
вы можете столкнуться с данной проблемой.    
В клиенте вы прописали прямоугольник, задали ему длины сторон и посчитали площадь,  
но что делать если мы вместо прямоугольника подставим квадрат?  
Все также верно посчитает для квадрата, 
но мы ожидали результат как при поведении базового класса.  

Пример: задали прямоугольник (4; 12)
Посчитали его площадь, получили 48
Вместо прямоугольника подставим квадрат и запустим код.
Получаем 144, что явно не ожидаем от класса наследника, ведь мы ожидаем те же 48,
потому что прямоугольник - это квадрат. Возникает вопрос: почему 144?
Я забыл уточнить, что в поля ширины и высоты квадрата добавились сеттеры,
которые при получении любой длины стороны автоматически присваивали
обоим полям (ширина, высота) данное число, это же квадрат все же :D

В таких случаях общие свойства и методы выносят в абстрактный класс,  
а в самих потомках переопределяют абстрактные методы и поля.  
Таким образом базовый абстрактный класс Фигура имеет одинаковое поведение, что и потомки => проблема решена!  
```
